...
skipped_directories = []
skipped_files = []
warn_on_skipped_files = (
    os.environ.get("PORTAGE_DOHTML_WARN_ON_SKIPPED_FILES") is not None
)
unwarned_skipped_extensions = os.environ.get(
    "PORTAGE_DOHTML_UNWARNED_SKIPPED_EXTENSIONS", ""
).split()
unwarned_skipped_files = os.environ.get(
...
def install(basename, dirname, options, prefix=""):
    ...
    if not os.path.exists(fullpath):
        ...
        if ext in options.allowed_exts or basename in options.allowed_files:
            ...
            warn_on_skipped_files
            and ext not in unwarned_skipped_extensions
